...
void DemoInitialize() {
   ...
   XGpio_Initialize(&VgaGpio, XPAR_AXI_GPIO_0_DEVICE_ID);
   XGpio_SetDataDirection(&VgaGpio, 1, 0);
   XGpio_DiscreteWrite(&VgaGpio, 1, 0);
}
...
void DemoRun() {
   ...
   while (1) {
      ...
      VgaData = (data.r & 0xF800);
      VgaData |= (data.g >> 5) & 0x07E0;
      VgaData |= (data.b >> 11) & 0x001F;

      XGpio_DiscreteWrite(&VgaGpio, 1, VgaData);
